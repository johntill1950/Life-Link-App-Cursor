...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const checkVitals = async () => {
      if (
        ...
        // Only create a new alert if there isn't one active
        if (!activeAlert) {
          try {
        ...
        // Reset active alert if vitals are back to normal
        if (activeAlert) {
          setActiveAlert(null);
        }
      }
  ...
  return (
    <div className="min-h-screen bg-blue-50 dark:bg-gray-900 p-4 space-y-6 dashboard-container">
      <AuthForm />
      <NotificationButton />
      <AlertStatus activeAlertId={activeAlert} onAlertCancelled={handleAlertCancelled} />
      <NotificationManager />
